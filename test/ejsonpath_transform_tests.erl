...
key_access_test() ->
    ...
    ?assertError(not_found,
        ejsonpath_transform:transform(?AST("$.b"),
                                      #{<<"a">> => x},
                                      fun ({match, #{node := x}}) -> {ok, y} end, #{}, [])),

    ?assertEqual({#{<<"a">> => #{<<"a">> => y}},  ["$['a']['a']"]},
        ejsonpath_transform:transform(?AST("$.a.a"),
                                      #{<<"a">> => #{<<"a">> => x}},
                                      fun ({match, #{node := x}}) -> {ok, y} end, #{}, [])),
    ok.
...
index_access_test() ->
    ?assertEqual({#{<<"a">> => [y, y, z]},  ["$['a'][0]"]},
        ejsonpath_transform:transform(?AST("$.a[0]"),
                                      #{<<"a">> => [x, y, z]},
                                      fun ({match, #{node := x}}) -> {ok, y} end, #{}, [])),
    ?assertEqual({#{<<"a">> => [x, y, y]},  ["$['a'][-1]"]}, % TODO: try and put the derived index
        ejsonpath_transform:transform(?AST("$.a[-1]"),
                                      #{<<"a">> => [x, y, z]},
                                      fun ({match, #{node := z}}) -> {ok, y} end, #{}, [])),

    ?assertError(not_found,
        ejsonpath_transform:transform(?AST("$.a[3]"),
                                      #{<<"a">> => [x, y, z]},
                                      fun (_) -> {ok, y} end, #{}, [])),
...
access_list_test() ->
    ?assertEqual(
        ...
        ejsonpath_transform:transform(?AST("$['a', 'b']"),
                                      #{<<"a">> => yyy, <<"b">> => yyy, <<"c">> => yyy},
                                      fun (_) -> {ok, xxx} end, #{}, [])),

    ?assertError(not_found,
        ejsonpath_transform:transform(?AST("$['a', 'd']"),
                                      #{<<"a">> => yyy, <<"b">> => yyy, <<"c">> => yyy},
                                      fun (_) -> {ok, xxx} end, #{}, [])),
    ok.
...
multimatch_test() ->
    ...
    ?assertEqual(
        ...
        ejsonpath_transform:transform(?AST("$.nums.*"),
                                      O1,
                                      fun ({match, #{type := array, node := E}}) ->
                                              {ok, E ++ [hd(lists:reverse(E)) + 1]}
                                      end, #{}, [])),
    ...
    ?assertEqual({#{<<"outer">> =>
                  ...
                  ["$['outer'][0]['inner'][0]['id']",
                   "$['outer'][1]['inner'][0]['id']"]},
        ejsonpath_transform:transform(?AST("$.outer.*.inner[0].id"),
                                      O2,
                                      fun ({match, #{node := a}}) -> {ok, b} end, {ok, #{}}, [])),
    ok.
...
delete_test() ->
    ...
    ?assertEqual({O, ["$['c']"]},
        ejsonpath_transform:transform(?AST("$.c"),
                                      O,
                                      fun ({not_found, _, _, _}) -> delete end, #{},
                                      [handle_not_found])),
    ...
    ?assertError(not_found,
        ejsonpath_transform:transform(?AST("$.b.c"),
                                      O1,
                                      fun ({not_found, _, _, _}) -> delete end, #{}, [])),
    ?assertError(not_found,
        ejsonpath_transform:transform(?AST("$.b.c"),
    ...
    ?assertError(not_found,
        ejsonpath_transform:transform(?AST("$.items[?(@.id == 10)]"), O3, fun (_) -> delete end,
                                      #{}, [handle_not_found])),
